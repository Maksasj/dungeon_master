...
void initBackground(u16* _pallete, u16* _data, i32 _width, i32 _height) {
    ...
    memcpy16DMA((u16*) charBlock(0), (u16*) _data, (_width * _height) / 2);
    ...
    *_BG1_CONTROL_ = 1 |            // priority, 0 is highest, 3 is lowest
        ...
        (0 << 6)  |                 // the mosaic flag
        (1 << 7)  |                 // color mode, 0 is 16 colors, 1 is 256 colors
        (27 << 8) |                  // the screen block the tile data is stored in
        (1 << 13) |                 // wrapping flag
        (0 << 14);                  // bg size, 0 is 256x256
    ...
    *_BG2_CONTROL_ = 0 |            // priority, 0 is highest, 3 is lowest
        ...
        (0 << 6)  |                 // the mosaic flag
        (1 << 7)  |                 // color mode, 0 is 16 colors, 1 is 256 colors
        (29 << 8) |                  // the screen block the tile data is stored in
        (1 << 13) |                 // wrapping flag

...
void initTimer(Timer *_timer) {
    _timer->startTimestamp   = -1;
    _timer->stopTimestamp    = -1;
}

void startTimer(Timer *_timer) {
    _timer->startTimestamp = *_TIMER_3_DATA_;
}

void stopTimer(Timer *_timer) {
    _timer->stopTimestamp = *_TIMER_3_DATA_;
}
...
ivec3 formatTime(Timer *_timer) {
    if(_timer->startTimestamp == -1) {
        return (ivec3){.x = 0, .y = 0, .z = 0};
    }

    i32 tmpStopTimestamp = _timer->stopTimestamp;

    if(tmpStopTimestamp == -1) {
        tmpStopTimestamp = *_TIMER_3_DATA_;
    }
    
    i32 delta = tmpStopTimestamp - _timer->startTimestamp;

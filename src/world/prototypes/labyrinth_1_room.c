#include "../../../include/world/prototypes/labyrinth_1_room.h"

void labyrinth1RoomInit(Room* _self, Entity* _player) {
    _self->type = LABYRINTH_1_ROOM;
    ...
    if(class == WARRIOR)
        ...
        tryPushItemDropToRoom(_self, _FIRE_STAFF_ITEM_DROP_(48, 128));
    else if(class == ARCHER)
        tryPushItemDropToRoom(_self, _AQUA_BOW_ITEM_DROP_(48, 128));

    tryPushLightToRoom(_self, (ivec2){.x = 112, .y = 0});
    tryPushLightToRoom(_self, (ivec2){.x = 192, .y = 32});
    tryPushLightToRoom(_self, (ivec2){.x = 32, .y = 112});
}
...
void labyrinth1RoomRender(Room* _self, void* _world) {
    ...
    placeTile(_world, TILE_BACKGROUND_SCREEN_BLOCK, newIVec2(12, 2), TORCH_TILE, NONE);
    placeTile(_world, TILE_BACKGROUND_SCREEN_BLOCK, newIVec2(2, 7), TORCH_TILE, NONE);
}
